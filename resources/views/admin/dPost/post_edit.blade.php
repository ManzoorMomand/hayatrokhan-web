...
@section('button')
<a href="{{ route('admin_d_post_show_ajax')}}" class="btn btn-primary"><i class="fas fa-eye"></i> view</a>
@endsection
<script src="https://cdn.jsdelivr.net/npm/axios/dist/axios.min.js"></script>

<div class="section-body">
<form action="{{ route('admin_d_post_update_ajax',$posts_EDIT->id)}}" method="post" enctype="multipart/form-data">
    @csrf
                                ...
                                <div class="card-body">
                                         <div class="form-group mb-3">
                                            ...
                                            <label>sub_category</label>
                                               <select name="d_sub_category_id" class="form-control">
                                                @foreach($sub_categories as $item)
                                                <option value="{{$item->id}}" @if($item->id == $posts_EDIT->sub_category_id) selected @endif>
                                                    {{$item->d_sub_category_name}} ({{$item->rCategory->category_name}})
                                                </option>
                                                    @endforeach
                                               </select>
                                            </div>
